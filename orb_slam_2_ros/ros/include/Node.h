...
class Node
{
  ...
  private:
    void PublishLocalMapPoints (std::vector<ORB_SLAM2::MapPoint*> map_points);
    ...
    void PublishKeyFrames(ros::Publisher pub, const vector<ORB_SLAM2::KeyFrame*> &vpKFs);

    ros::Publisher keyFrame_publisher;
    ros::Publisher local_keyFrame_publisher;
    ros::Publisher candidate_keyFrame_publisher;

    // initialization Transform listener
    boost::shared_ptr<tf2_ros::Buffer> tfBuffer;
    boost::shared_ptr<tf2_ros::TransformListener> tfListener;

    cv::Mat Mat_TransformFromMat (cv::Mat position_mat);
    ...
    ros::Publisher local_map_points_publisher_;
    ...
    ros::Subscriber lidar_pose_subscriber;
    bool ifInitStaticTfPub;
    void LidarPoseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg);

    double target_camera_x;
    double target_camera_y;
    double target_camera_z;
};
