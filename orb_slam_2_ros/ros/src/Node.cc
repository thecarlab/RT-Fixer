...
void Node::Init () {
  ...
  if (publish_pointcloud_param_) {
    ...
    local_map_points_publisher_ = node_handle_.advertise<sensor_msgs::PointCloud2> (name_of_node_+"/local_map_points", 1);
  }
  ...
  ifInitStaticTfPub = false;
  lidar_pose_subscriber = node_handle_.subscribe ("/ndt_matching_sample/ndt_pose", 10, &Node::LidarPoseCallback, this);

  keyFrame_publisher = node_handle_.advertise<geometry_msgs::PoseArray>("KeyFrames", 10);
  local_keyFrame_publisher = node_handle_.advertise<geometry_msgs::PoseArray>("LocalKeyFrames", 10);
  candidate_keyFrame_publisher = node_handle_.advertise<geometry_msgs::PoseArray>("candidateKeyFrames", 10);

  PublishKeyFrames(keyFrame_publisher, orb_slam_->GetMap()->GetAllKeyFrames());
  PublishKeyFrames(local_keyFrame_publisher, orb_slam_->GetTracker()->mvpLocalKeyFrames);
  PublishKeyFrames(candidate_keyFrame_publisher, orb_slam_->GetTracker()->vpCandidateKFs);
  /*
  ...
  geometry_msgs::TransformStamped base_link_to_camera_link;
  bool received_tf = true;
  while(1){
    try{
      received_tf = true;
      base_link_to_camera_link = tf_Buffer.lookupTransform(target_frame_id_param_.c_str(), camera_frame_id_param_.c_str(), ros::Time(0), ros::Duration(3.0) );
      break;
    }
  ...
  target_camera_x = base_link_to_camera_link.transform.translation.x;
  target_camera_y = base_link_to_camera_link.transform.translation.y;
  target_camera_z = base_link_to_camera_link.transform.translation.z;
  */
}

cv::Mat tf_Mat(tf2::Transform _tf)
{
  //tf2::Transform tf_pose = TransformFromMat(vpKFs[i]->GetPose());
        
  cv::Mat Tcw = cv::Mat::eye(4, 4, CV_32F);
  tf2::Matrix3x3 Rot = _tf.getBasis ();
  tf2::Vector3 trans = _tf.getOrigin ();
        
  for(int i = 0; i < 3; i++)
  {
    tf2::Vector3 test = Rot.getRow(i);

    Tcw.at<float>(i, 0) = (float)test.getX();
    Tcw.at<float>(i, 1) = (float)test.getY();
    Tcw.at<float>(i, 2) = (float)test.getZ();
  }

  Tcw.at<float>(0, 3) = (float)trans.getX();
  Tcw.at<float>(1, 3) = (float)trans.getY();
  Tcw.at<float>(2, 3) = (float)trans.getZ();

  return Tcw;
}

void Node::PublishKeyFrames(ros::Publisher pub, const vector<ORB_SLAM2::KeyFrame*> &vpKFs)
{
    ...
    for(size_t i=0, iend=vpKFs.size() ;i<iend; i++){
      ...
      tf2::toMsg(TransformFromMat(vpKFs[i]->GetPose()), pose_msg); 
...
void Node::Update () {
  ...
  if (publish_pointcloud_param_) {
    ...
    PublishLocalMapPoints (orb_slam_->GetTrackedMapPoints());
  }
...
void Node::PublishLocalMapPoints (std::vector<ORB_SLAM2::MapPoint*> map_points) {
  sensor_msgs::PointCloud2 cloud = MapPointsToPointCloud (map_points);
  local_map_points_publisher_.publish (cloud);
}
...
sensor_msgs::PointCloud2 Node::MapPointsToPointCloud (std::vector<ORB_SLAM2::MapPoint*> map_points) {
  ...
  cout << cloud.width << " " << cloud.point_step << " "<< map_points.size() << endl;
  for (unsigned int i=0; i<cloud.width; i++) {
    if(map_points.at(i) == NULL)
      continue;
...
void Node::LidarPoseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg){
  
  if(orb_slam_->GetTrackingState() == 1 && !ifInitStaticTfPub){
    ROS_INFO("Tracking Status is %d, init status is %d", orb_slam_->GetTrackingState(), ifInitStaticTfPub);
    ifInitStaticTfPub = true;
    tf2_ros::Buffer tf_Buffer;
    tf2_ros::TransformListener tf_Listener(tf_Buffer);
    ...
    geometry_msgs::TransformStamped base_link_to_camera_link;
    base_link_to_camera_link = tf_Buffer.lookupTransform("base_link", "camera_link", ros::Time(0), ros::Duration(3.0) );
    geometry_msgs::Pose new_pose = msg->pose;
    new_pose.orientation.x = 0.0;
    new_pose.orientation.y = 0.0;
    new_pose.orientation.z = 0.0;
    new_pose.orientation.w = 1.0;
    ROS_INFO("[x]: %lf, [y]: %lf, [z]: %lf", new_pose.position.x, new_pose.position.y, new_pose.position.z);
    tf2::doTransform(new_pose, new_pose, base_link_to_camera_link);
    ROS_INFO("[x]: %lf, [y]: %lf, [z]: %lf", new_pose.position.x, new_pose.position.y, new_pose.position.z);
    static_transformStamped.header.stamp = ros::Time::now();
    static_transformStamped.header.frame_id = "world";
    static_transformStamped.child_frame_id = "map";
    static_transformStamped.transform.translation.x = new_pose.position.x;
    static_transformStamped.transform.translation.y = new_pose.position.y;
    static_transformStamped.transform.translation.z = new_pose.position.z;
    static_transformStamped.transform.rotation.x = new_pose.orientation.x;
    static_transformStamped.transform.rotation.y = new_pose.orientation.y;
    static_transformStamped.transform.rotation.z = new_pose.orientation.z;
    static_transformStamped.transform.rotation.w = new_pose.orientation.w;
