...
class SyncNode:
    ...
    def sync_callback(self, point_cloud, odom):
        ...
        rospy.loginfo(f"Pre save buffer size {len(self.pre_save_buffer)}")
        rospy.loginfo(f"Post save buffer size {len(self.post_save_buffer)}")

        if not self.save_triggered:
            # Add new messages to the pre-save buffer
            self.pre_save_buffer.append((point_cloud, odom))
            if len(self.pre_save_buffer) > 40:
    ...
    def generateLocalMap(self):
        ...
        transformed_redundancy_points = self.transform_point_cloud(self.redundancy_cloud, self.redundancy_pose)
        transformed_redundancy_cloud_msg = create_point_cloud_msg(transformed_redundancy_points)
